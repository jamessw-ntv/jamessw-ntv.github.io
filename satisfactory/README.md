...
| Tab | What it does |
|-----|--------------|
| **✅ Do Next** | The numbered **step-by-step list** (HUB → launch). **🎮 Each time you open it**, a check-in asks "are these last few builds still standing?" (✓ / ✗ puts one back on the list; "I'm further ahead" jumps you forward), then **🎯 Focus mode** shows just the current step, big, with a picture of what to place (input belts → machines to place now / room to leave → output and where it goes), a power bar and the next three steps. Every step comes after what it needs: the 🔓 milestone or MAM research that unlocks it (with its real cost), the district pad and its utilities, its suppliers, ⚡ generators (including automatic top-ups before the grid would trip), and one **➕ Expand** step per bank as later phases need more. ↶ Undo fixes a mis-tap. |
